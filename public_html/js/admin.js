...
$(document).ready(function() {	
	tinyMCE.init( {
		...
		theme : "advanced",
		skin : "o2k7",
		plugins : "safari,spellchecker,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,youtube",
		// Theme options
		theme_advanced_buttons1 : "save,newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,styleselect,formatselect,fontselect,fontsizeselect",
		theme_advanced_buttons2 : "cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,image,youtube,cleanup,help,code,|,insertdate,inserttime,preview,|,forecolor,backcolor",
...
var dialogSelectPosition = (function(dialog,location,id,cor) {
	...
	$('#img_src  > img').attr('src',YELLOW_SERVER+'location/'+location+'.jpg');
	$('#img_src  > img').attr('width',width);
...
var ajaxGetCategory = (function(category,objId,index) {
	...
	if (category == '0') {
		...
		$.get("ajax/getpathofcategory", {category : category,index:index}, function(data) {
			$(objId).append(data);
		});
	}
});
...
var dialogEditCategories = (function(obj,dialog) {
	$.get(obj.attr('href'), function(data) {
		...
		$("#"+dialog+"-form select[name=parent_id]").val(data.parent_id);
		$("#"+dialog+"-form select[name=active]").val(data.active);
		$("#"+dialog+"-form select[name=hightlight]").val(data.hightlight);
		$("#"+dialog+"-form select[name=homepage]").val(data.homepage);		
		$('#'+dialog).dialog('open');
		$.each(data.editor.split(','), function(val, text) {
	  		$('#'+dialog+'-editor-'+text).attr('checked', true);
		});
		$.each(data.manager.split(','), function(val, text) {
	  		$('#'+dialog+'-manager-'+text).attr('checked', true);
		});
		
		
	}, "json");
});
var doAddPart = (function(obj){
	i = parseInt($('[name=split_controler]').val())+1;
	$('[name=split_controler]').attr('value',i);
	$("#"+obj).append('Link đến phân đoạn dưới này bằng liên kết: <b><font color="red">?page='+i+'</font></b><br><textarea name="description[]" id="description_'+i+'"></textarea>');
	tinyMCE.execCommand('mceAddControl', false, 'description_'+i);
});
...
var initAutoComplete = (function(link,obj,callback){
	$(obj).autocomplete({
		source: function( request, response ) {
		$.ajax({
			url: DOMAIN_NAME + link,
			dataType: "json",
		...
		minLength: MINLENGTH_AUTOCOMPLETE,
		select: function(event, ui) {		
			$.each(callback, function(val, text) {
		  		$(val).val(ui.item[text]);		  		
			});			
		}
	});
});
...
var dialogEditCountry = (function(obj) {
	$.get($(obj).attr('href'),
			function(data) {
				...
				$("#dialog-form-edit input[name=sort_order]").val(data.sort_order);
				...
				$("#dialog-form-edit input[name=seo_meta]").val(data.seo_meta);
				$("#dialog-form-edit input[name=seo_keyword]").val(data.seo_keyword);
				$("#dialog-form-edit select[name=visa]").val(data.visa);
				$("#dialog-form-edit textarea[name=visa_requirements]").val(data.visa_requirements);
				$("#dialog-form-edit select[name=tour]").val(data.tour);
...
var dialogEditLocation = (function(obj) {
	$.get($(obj).attr('href'), function(data) {
		$("#dialog-form-edit input[name=id]").val(data.id);
		$("#dialog-form-edit input[name=url]").val($(obj).attr('href'));
		$("#dialog-form-edit img[name=anh]").attr('src',YELLOW_SERVER + 'location/'+data.id+'.jpg');
		$("#dialog-form-edit input[name=name]").val(data.name);
		$("#dialog-form-edit input[name=sort_orders]").val(data.sort_orders);
		$("#dialog-form-edit select[name=status]").val(data.status);
		$("#dialog-form-edit input[name=map]").val(data.map);
