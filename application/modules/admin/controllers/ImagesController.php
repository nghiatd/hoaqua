...
class Admin_ImagesController extends Zend_Controller_Action {
	private function _uploadFiles($uploadDir = '',$id) {
		...
		$images->addFilter ( 'Rename', array ('target' => $uploadDir . DIRECTORY_SEPARATOR . $id.'.jpg', 'overwrite' => true ) );
	...
	public function createAction() {
		...
		if ($this->getRequest ()->isPost ()) {
			...
			if ($music->trySave ()) {
				...
				//$this->Member->log ( 'Create Room: ' . $News->name . '(' . $News->id . ')', 'News' );
	...
	public function editAction() {
		...
		//	print_r($_FILES['file']['name']);
			...
			if (count ( $error ) == 0) {
				$music->merge ( $request );
				if ($music->trySave ()) {
					//$this->Member->log ( 'Chỉnh sửa tin ' . $Room->name . '(' . $Room->id . ')', 'Room' );
